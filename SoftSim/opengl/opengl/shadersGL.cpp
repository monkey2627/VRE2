...
void main()
{
	...
	{
		//gl_TexCoord[3].xyz = mix(vec3(0.354f, 0.797f, 0.000f), vec3(1.0), 0.1);
		gl_TexCoord[3].xyz = mix(vec3(0.9f, 0.0f, 0.000f), vec3(1.0), 0.1);
	}
...
void DrawPoints(FluidRenderBuffers* buffersIn, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowMap, bool showDensity)
{
	...
	if (sprogram)
	{
		...
		if (d != -1)
		{
			...
			glVerify(glVertexAttribPointer(d, 1,  GL_FLOAT, GL_TRUE, 0, 0));	// densities
		}
...
void  DrawMesh(float* vert, float* norm, float* color, unsigned int* tri,  int triNum) {
	Vec3 color1(0.5);

	glVerify(glColor3fv(color1));
	glVerify(glSecondaryColor3fv(color1));
	...
	glVerify(glNormalPointer(GL_FLOAT, sizeof(float) * 3, norm));
	glVerify(glVertexPointer(3, GL_FLOAT, sizeof(float) * 3, vert));

	if (color) {
		glVerify(glEnableClientState(GL_COLOR_ARRAY));
		glVerify(glColorPointer(4, GL_FLOAT, 0, color));
		//glVerify(glEnableClientState(GL_COLOR_ARRAY));
		//glVerify(glColorPointer(3, GL_FLOAT, 0, norm));
	}

	glVerify(glDrawElements(GL_TRIANGLES, triNum, GL_UNSIGNED_INT, tri));
...
FluidRenderBuffers* CreateFluidRenderBuffers(int numFluidParticles, bool enableInterop)
{
	...
	glVerify(glBufferData(GL_ARRAY_BUFFER, sizeof(float)*numFluidParticles, 0, GL_DYNAMIC_DRAW));
...
void UpdateFluidRenderBuffers(FluidRenderBuffers* buffersIn,
	float* particles,	int numParticles, int* indices, int numIndices)
{
	FluidRenderBuffersGL* buffers = reinterpret_cast<FluidRenderBuffersGL*>(buffersIn);
	// regular particles
	glVerify(glBindBuffer(GL_ARRAY_BUFFER, buffers->mPositionVBO));
	glVerify(glBufferSubData(GL_ARRAY_BUFFER, 0, buffers->mNumParticles * sizeof(float)*3, particles));
	...
	int* indices, int numIndices)
{
	...
	glVerify(glBindBuffer(GL_ARRAY_BUFFER, buffers->mDensityVBO));
	glVerify(glBufferSubData(GL_ARRAY_BUFFER, 0, buffers->mNumParticles * sizeof(float), intensity));
...
void DemoContextOGL::drawMesh(float* vert, float* norm, float* color, unsigned int* tri, int triNum)
{
	OGL_Renderer::DrawMesh(vert, norm, color, tri, triNum);
}
...
void DemoContextOGL::updateFluidRenderBuffers(FluidRenderBuffers* buffers, float* particles, int numParticles, int* indices, int numIndices)
{
	OGL_Renderer::UpdateFluidRenderBuffers(buffers, particles, numParticles, indices, numIndices);
}

void DemoContextOGL::updateFluidRenderBuffers(FluidRenderBuffers* buffers, float* particles, int numParticles, float* density, int* indices, int numIndices)
{
	OGL_Renderer::UpdateFluidRenderBuffers(buffers, particles, numParticles, density, indices, numIndices);
}
