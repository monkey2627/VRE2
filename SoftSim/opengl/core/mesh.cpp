            ...
            for (int i=0; i < 4; ++i)
            {
				...
				if (!file.eof())
				{
					...
					else
					{
						...
						m->m_colours.push_back(Colour(0.0f, 1.0f, 0.0f));
...
void Mesh::ExportToOBJ(const char* path) {
	ofstream file(path);
	...
	file << "# positions" << endl;
	for (uint32_t i = 0; i < m_positions.size(); ++i)
		file << "v " << m_positions[i].x << " " << m_positions[i].y << " " << m_positions[i].z << endl;

	file << "# faces" << endl;
	for (uint32_t i = 0; i < m_indices.size() / 3; ++i)
		file << "f " << m_indices[i*3]+1 << " " << m_indices[i * 3+1] + 1 << " " << m_indices[i * 3+2] + 1 << endl;
